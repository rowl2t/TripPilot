...
export const useEntitlement = () =>
  useQuery({ queryKey: ['entitlement'], queryFn: async () => (await resolveEntitlement(mobileAuthClient, USER_ID)).data });

export const useUsage = () =>
  useQuery({ queryKey: ['usage'], queryFn: async () => (await getUsageSnapshot(mobileAuthClient, USER_ID)).data });
...
export const usePaywallActions = () => {
  const rc = createRevenueCatAdapter(process.env.REVENUECAT_API_KEY);
  return {
